...
import type { Enrollment } from '@/stores/courses'

// 타입 정의
type FilterType = 'all' | 'active' | 'completed' | 'paused'
type SortType = 'recent' | 'enrolled' | 'progress' | 'name'
...
const totalWatchTime = computed(() => {
  const totalMinutes = enrolledCourses.value.reduce((sum, course) =>
    sum + ((course as any).watchTime || 0), 0
  )
...
const handleFilterChange = (filterName: string | number) => {
  activeFilter.value = filterName as FilterType
}
...
const viewCertificate = (enrollment: Enrollment) => {
  selectedCertificate.value = {
    ...
    totalWatchTime: (enrollment as any).watchTime || 0
  }
  certificateDialogVisible.value = true
}

const pauseCourse = async (enrollment: Enrollment) => {
  try {
...
const resumeCourse = async (enrollment: Enrollment) => {
  try {
